...
```{}
####################################################################
Uses new CRM data provided by Jana. More complete cases
Same as payments, recalculated with houshold-level id.
Replaced nf_customer_key (individual) with nf_parent_key (household)
####################################################################
```
...
#Read Data
```{r}
d <- fread("payments2.csv") %>% 
  mutate(payment_date = as.Date(payment_date),
         nf_customer_key = nf_parent_key)
...
year_uniques$`Percent New` <- round(year_uniques$new / year_uniques$uniqes, 4)
year_uniques$churn_ma <- cumsum(c(0,na.omit(year_uniques$`Churn Rate`))) / 1:length(year_uniques$`Churn Rate`)
```

```{r, fig.height=3, fig.width=10}
ggplot(year_uniques %>% 
         filter(fyear %in% c(2008:2017)) %>%
         gather(metric, val, c(`Churn Rate`:`Acquisition Rate`)),
       aes(x = fyear,
           y = val,
           color = metric)) +
  labs(title = "Churn and Acquisition Rates. FY2008-FY2017.",
       x = "",
       y = "",
       color = "") +
  geom_point(size = 1.8) +
  ...
  scale_x_continuous(breaks = 2008:2017,
                     labels = paste("FY", 8:17, sep = "")) +
  scale_color_manual(values = c("#109dcc", "#cc4b10", "#ccb910")) +
  my_theme +
  theme(legend.position = c(0.77, 0.79),
...
ggplot(year_uniques %>% 
         filter(fyear %in% c(2008:2017)) %>%
         gather(metric, val, lost:new),
       aes(x = fyear,
           y = val,
           color = metric)) +
  labs(title = "Donor Tally. FY2008-FY2017.",
       x = "",
       y = "",
       color = "") +
  geom_point(size = 1.8) +
  ...
  scale_x_continuous(breaks = 2008:2017,
                     labels = paste("FY", 8:17, sep = "")) +
  scale_color_manual(values = c("#cc4b10", "#109dcc",  "#ccb910")) +
  my_theme +
...
```
#New stacked bar plot for new, renew and rejoined
```{r, fig.height=3, fig.width=10}
trends_jana <- fread("year_trends_jana.csv")
ggplot(a <- trends_jana %>% 
   filter(`fiscal year` %in% c(2008:2017)) %>%
     gather(metric, val, c(new, renew:rejoin)) %>%
     group_by(`fiscal year`) %>%
     mutate(metric = factor(metric, levels = c("new", "rejoin", "renew"))) %>%
     arrange(`fiscal year`, desc(metric)) %>% 
     mutate(pos = cumsum(val) - (0.5 * val ))) +
  labs(title = "New Donor Breakdown by Financial Year",
       x = "",
       y = "",
       fill = "") +
  ...
  geom_text(aes(x = `fiscal year`,
                y = pos,
                label = scales::comma(val)),
            size = 3,
            color = "black") +
  scale_fill_manual(values = c("#ccb910", "#cc4b10", "#109dcc")) +
  scale_x_continuous(breaks = 2008:2018,
                     labels = paste("FY", 08:18, sep = "")) +
  scale_y_continuous(labels = scales::comma) +
  my_theme +
  theme(legend.position = c(0.115,0.8))
